...
export default function AddressesPage() {
  ...
  return (
    <div className="p-4 max-w-4xl mx-auto">
      <div className="flex justify-between gap-2 items-center mb-8">
        ...
        {!showAddForm && !editingAddress && (
          <Button
            ...
            className="px-3 lg:px-4 text-wrap py-2"
          >
      ...
      {/* Add/Edit Form */}
      {showAddForm && (
        <div className="bg-white shadow p-6 mb-8 border">
      ...
      {addresses.length === 0 && !showAddForm && !editingAddress ? (
        <div className="bg-white shadow p-8 text-center border">
          <DynamicIcon
            name="MapPin"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {addresses.map((address) => (
            <div
              key={address.id}
              className="bg-white shadow p-5 border relative"
            >
              {address.isDefault && (
                <div className="absolute top-3 right-3">
                  <span className="bg-primary/10 text-primary text-xs font-semibold px-2 py-1 flex items-center">
                    <DynamicIcon name="Check" className="h-3 w-3 mr-1" />
                    Default
                  </span>
                </div>
              )}
              ...
              <div className="flex gap-2 flex-wrap">
                {!address.isDefault && (
                  ...
                  >
                    <DynamicIcon name="Home" className="h-4 w-4 mr-2" />
                    Set as Default
                  </Button>
                )}
                ...
                >
                  <DynamicIcon name="Edit" className="h-4 w-4 mr-2" />
                  Edit
                </Button>
                ...
                >
                  {deletingId === address.id ? (
                    <div className="animate-spin h-4 w-4 mr-2 border-b-2 border-red-500"></div>
                  ) : (
