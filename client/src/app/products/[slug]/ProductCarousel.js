...
export default function ProductCarousel({
  ...
  if (!images || images.length === 0) {
    return (
      <div className="relative h-[450px] w-full bg-gray-100 flex items-center justify-center border mb-6">
        <Image
    ...
    <div className="bg-white overflow-hidden mb-6 border border-gray-200">
      ...
      >
            ...
            <CarouselItem key={index}>
              <div className="relative h-[450px] w-full bg-gray-50 transition-all duration-300">
                ...
                {showSaleBadge && index === 0 && (
                  <div className="absolute top-4 left-4 bg-primary text-white text-sm font-bold px-4 py-1">
                    SALE
                  </div>
                )}
              </div>
        ...
        <button
          ...
          className="absolute bottom-4 right-4 h-8 w-8 bg-white/70 hover:bg-white flex items-center justify-center z-10 shadow-sm"
      ...
      {images.length > 1 && (
        <div className="mt-4 px-4 relative">
            ...
            <div className="embla-thumbs" ref={thumbViewportRef}>
              <div className="flex space-x-3 py-2">
                {images.map((image, index) => (
                  <div
                    key={index}
                    className={`relative flex-shrink-0 cursor-pointer overflow-hidden border-2 transition-all hover:shadow-md ${
                      selectedSlide === index
                        ? "border-primary"
          ...
          {images.length > 5 && (
            <>
              <button
                className="absolute left-0 top-1/2 -translate-y-1/2 h-8 w-8 flex items-center justify-center bg-white hover:bg-gray-100 shadow-md z-10 border border-gray-200"
              ...
              <button
                className="absolute right-0 top-1/2 -translate-y-1/2 h-8 w-8 flex items-center justify-center bg-white hover:bg-gray-100 shadow-md z-10 border border-gray-200"
